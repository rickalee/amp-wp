...
if ! [[ $TRAVIS_BRANCH =~ ^[a-z][a-z0-9-]{0,10}$ ]] || [[ $TRAVIS_BRANCH =~ ^(live|test|master|settings|team|support|debug|multidev|files|tags|billing)$ ]]; then
    echo "Aborting since branch '$TRAVIS_BRANCH' cannot be an environment."
    exit 1
fi
